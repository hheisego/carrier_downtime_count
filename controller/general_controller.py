...
def super_main():
    ...
    app_logger.debug(f"{__name__} Starting super_main...")
    ...
    if active_alerts:
        app_logger.debug(f"{__name__} Found {len(active_alerts)} active alerts.")

        #3. Get details for each alert
        carriers_metrics = get_carriers_metrics(active_alerts)


    app_logger.debug(f"{__name__} Finished super_main.")